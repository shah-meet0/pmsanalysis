...
def on_manager_selection(manager_selected, _analysed_data, _monthly_data, index_returns, interpretation_flag):
    ...
    st.write(f'WARNING: Wealth evolution and Returns both have forecasts from Aug 2020 to Mar 2021.')
...
if manager == 'All':
    ...
    if data_processing_flag:
        ...
        st.write(
            'Where needed, risk free rate was taken as 6.24%, the 10 year government bond yield at time of writing.')
    ...
    if not hide_data_flag:
        ...
        st.dataframe(analysed_data.style.background_gradient(cmap='gist_gray', axis=0).
                     format(precision=3, formatter={'Ann_Ret': "{:.2%}", 'Ann_Vol': "{:.2%}"}))
